...
if ($_GET['module'] == 'detail-post'){
	...
	echo "$detail[title] - Alumiku";
}

elseif ($_GET['module'] == 'all-categories'){
	echo "All Categories Forum - Alumiku";
}

elseif ($_GET['module'] == 'add-new-post'){
	echo "Add Your Post - Alumiku";
}
...
elseif ($_GET['module'] == 'cat_detail'){
	if ($_GET['cat'] != '0' AND $_GET['id'] != '0'){
		...
		echo "$data_frm_title[category_name] - $data_frm_title[sub_category] - Alumiku";
	}
	elseif ($_GET['cat'] == '0' AND $_GET['id'] != '0'){
		...
		echo "$data_frm_title[category_name] - Alumiku";
	}
	else{
		if ($_GET['pid'] != '0'){
			...
			echo "Forum Topics of $data_frm_title[region_name] - $data_frm_title[country_name] - $data_frm_title[province_name] - Alumiku";
		}
		else{
			if ($_GET['cid'] != '0'){
				$data_frm_title = $db->database_fetch_array($db->database_prepare("SELECT A.region_name, B.country_name, C.province_name FROM
				...
				echo "Forum Topics of $data_frm_title[region_name] - $data_frm_title[country_name] - Alumiku";
			}
			else{
				$data_frm_title = $db->database_fetch_array($db->database_prepare("SELECT A.region_name, B.country_name, C.province_name FROM
																			as_region A LEFT JOIN as_country B ON A.region_id=B.region_id
				...
				echo "Forum Topics of $data_frm_title[region_name] - Alumiku";
			}
		}
	}
}
	...
	echo "Forum - Connecting Middle East Societies - Alumiku";
}
?>
